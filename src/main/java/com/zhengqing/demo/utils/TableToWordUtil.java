...
public class TableToWordUtil {
     ...
     * @return: void
     */
    public void toWord(List<Tables> tables, String fileName, String title, List<Fields> allFields) {
        ...
        try {
            ...
            for (int i = 0; i < tables.size(); i++) {
                ...
                table.endHeaders();
                allFields.add(new Fields("主键", "id"));
                allFields.add(new Fields("创建时间", "created_at"));
                allFields.add(new Fields("更新时间", "updated_at"));
                allFields.add(new Fields("创建人", "creator_id"));
                allFields.add(new Fields("更新人", "updator_id"));
                allFields.add(new Fields("创建人部门", "creator_org_id"));
                for (TableFileds filed : fileds) {
                    for (Fields allField : allFields) {
                        if (allField.getKey().equals(filed.getField())) {
                            filed.setComment(allField.getName());
                            break;
                        }
                    }
                }
                ...
                for (int k = 0; k < fileds.size(); k++) {
                    ...
                    addContent(table, cell, fileds.get(k).getNull());
                    addContent(table, cell, StringUtils.isEmpty(fileds.get(k).getKey()) ? "否" : "是");
